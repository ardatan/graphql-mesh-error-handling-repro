            ...
            if (typeof subSchema === 'object') {
                ...
                if (subSchema.title) {
                    if (Object.keys(subSchema).length === 2 && subSchema.type) {
                        ...
                        if (existingSubSchema) {
                            if ((0, cross_inspect_1.inspect)(subSchema) === (0, cross_inspect_1.inspect)(existingSubSchema)) {
                ...
                if (typeof subSchema.additionalProperties === 'object') {
                    ...
                    if (additionalPropertiesKeys.length === 0 ||
                        ...
                        debugLogFn?.(`${path} has an empty additionalProperties object. So this is invalid. Replacing it with 'true'`);
                        subSchema.additionalProperties = true;
                    }
                }
                ...
                if (subSchema.description != null) {
                    ...
                    if (keys.length === 1) {
                        debugLogFn?.(`${path} has a description definition but has nothing else. Adding an object definition.`);
                        subSchema.type = 'object';
                        subSchema.additionalProperties = true;
                    }
                }
                ...
                if (subSchema.type === 'object' && subSchema.items) {
                    debugLogFn?.(`${path} has an object definition but with "items" which is not valid. So setting "items" to the actual definition.`);
                    const realSubschema = subSchema.items;
                    delete subSchema.items;
                    subSchema = realSubschema;
                }
                if (subSchema.properties && subSchema.type !== 'object') {
                    debugLogFn?.(`${path} has "properties" with no type defined. Adding a type property with "object" value.`);
                    subSchema.type = 'object';
                }
                if (typeof subSchema.example === 'object' && !subSchema.type) {
                    debugLogFn?.(`${path} has an example object but no type defined. Setting type to "object".`);
                    subSchema.type = 'object';
                }
                // Items only exist in arrays
                if (subSchema.items) {
                    debugLogFn?.(`${path} has an items definition but no type defined. Setting type to "array".`);
                    ...
                    if (Array.isArray(subSchema.items)) {
                        if (subSchema.items.length === 0) {
                            debugLogFn?.(`${path} has an items array with a single value. Setting items to an object.`);
                            subSchema.items = subSchema.items[0];
                        }
                        else {
                            debugLogFn?.(`${path} has an items array with multiple values. Setting items to an object with oneOf definition.`);
                            subSchema.items = {
                ...
                if (!subSchema.type) {
                    ...
                    if (Array.isArray(subSchema.required) &&
                        ...
                        !subSchema.anyOf &&
                        !subSchema.allOf) {
                        debugLogFn?.(`${path} has a required definition but no properties or oneOf/allOf. Setting missing properties with Any schema.`);
                        // Add properties
                        subSchema.properties = {};
                    ...
                    if (subSchema.properties ||
                        ...
                        debugLogFn?.(`${path} has properties or patternProperties or additionalProperties. Setting type to "object".`);
                        subSchema.type = 'object';
                    }
                    switch (subSchema.format) {
                        case 'int64':
                        case 'int32':
                            debugLogFn?.(`${path} has a format of ${subSchema.format}. Setting type to "integer".`);
                            subSchema.type = 'integer';
                            break;
                        case 'float':
                        case 'double':
                            debugLogFn?.(`${path} has a format of ${subSchema.format}. Setting type to "number".`);
                            subSchema.type = 'number';
                            break;
                        default:
                            if (subSchema.format != null) {
                                debugLogFn?.(`${path} has a format of ${subSchema.format}. Setting type to "string".`);
                ...
                if (subSchema.type === 'string' &&
                    ...
                    if (examples?.length) {
                        ...
                        if (format && format !== 'utc-millisec' && format !== 'style') {
                            debugLogFn?.(`${path} has a format of ${format} according to the example. Setting type to "string".`);
                ...
                if (subSchema.format) {
                    if (!JSONSchemaStringFormats.includes(subSchema.format)) {
                        debugLogFn?.(`${path} has a format of ${subSchema.format}. It should be one of ${JSONSchemaStringFormats.join(', ')}.`);
                    ...
                    if (subSchema.format === 'date-time' && subSchema.default != null) {
                        try {
                            if (new Date(subSchema.default).toJSON() !== subSchema.default) {
                                debugLogFn?.(`${path} has a default value of ${subSchema.default} but it is not a valid date-time.`);
                ...
                if (subSchema.required) {
                    if (!Array.isArray(subSchema.required) || subSchema.required.length === 0) {
                        debugLogFn?.(`${path} has a required definition but it is not an array. Removing it.`);
                        ...
                        for (const requiredProperty of subSchema.required) {
                            if (!(requiredProperty in subSchema.properties)) {
                                debugLogFn?.(`${path} has a required definition but it is not in properties. Adding it.`);
                                subSchema.properties[requiredProperty] = exports.AnySchema;
                            }
                        }
                    }
                }
                ...
                if (((subSchema.type === 'object' &&
                    ...
                    debugLogFn?.(`${path} has an example but no type defined. Setting type to ${subSchema.type}.`);
                    // if (typeof subSchema.additionalProperties === 'object') {
                ...
                if (subSchema.type === 'object' &&
                    ...
                    debugLogFn?.(`${path} has an empty properties object. Removing it and adding "additionalProperties": true.`);
                    delete subSchema.properties;
                    subSchema.additionalProperties = true;
                }
                if (subSchema.properties) {
                ...
                if (subSchema.default != null &&
                    ...
                    debugLogFn?.(`${path} has a default value as a JSON string for the type ${subSchema.type}. Converting it to JSON.`);
                    try {
