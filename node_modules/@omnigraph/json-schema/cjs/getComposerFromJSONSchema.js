...
const formatScalarMapWithoutAjv = {
    ...
    binary: scalars_js_1.GraphQLFile,
    'date-time': graphql_scalars_1.GraphQLDateTime,
    time: graphql_scalars_1.GraphQLTime,
    email: graphql_scalars_1.GraphQLEmailAddress,
    ipv4: graphql_scalars_1.GraphQLIPv4,
    ipv6: graphql_scalars_1.GraphQLIPv6,
    uri: graphql_scalars_1.GraphQLURL,
    uuid: graphql_scalars_1.GraphQLUUID,
    'unix-time': graphql_scalars_1.GraphQLTimestamp,
    int64: graphql_scalars_1.GraphQLBigInt,
...
function getComposerFromJSONSchema({ subgraphName, schema, logger, getScalarForFormat, }) {
    ...
    return (0, json_machete_1.visitJSONSchema)(schema, {
        enter(subSchema, { path, visitedSubschemaResultMap }) {
            ...
            if (subSchema.const) {
                ...
                let enumValueName = (0, utils_1.sanitizeNameForGraphQL)(subSchema.const.toString());
        ...
        leave(subSchemaAndTypeComposers, { path }) {
            ...
            if (subSchemaAndTypeComposers.anyOf) {
                ...
                for (const typeComposers of subSchemaAndTypeComposers.anyOf) {
                    ...
                    else if (outputTypeComposer instanceof graphql_compose_1.ObjectTypeComposer ||
                        outputTypeComposer instanceof graphql_compose_1.InterfaceTypeComposer) {
                        const typeElemFieldMap = outputTypeComposer.getFields();
                        for (const fieldName in typeElemFieldMap) {
                            // In case of conflict set it to JSON
                            // TODO: But instead we can convert that field into a union of all possible types
                ...
                case 'object':
                    ...
                    if (Object.keys(fieldMap).length === 0) {
                        ...
                        for (const requiredFieldName of subSchemaAndTypeComposers.required || []) {
                            const sanitizedFieldName = (0, utils_1.sanitizeNameForGraphQL)(requiredFieldName);
                            const fieldObj = output.getField(sanitizedFieldName);
                            if (!(0, graphql_1.isNonNullType)(fieldObj.type.getType()) && !fieldObj.extensions?.nullable) {
                                output.makeFieldNonNull(requiredFieldName);
                            }
                        }
                    }
                    ...
                    if (Object.keys(inputFieldMap).length === 0) {
                        ...
                        for (const requiredFieldName of subSchemaAndTypeComposers.required || []) {
                            const sanitizedFieldName = (0, utils_1.sanitizeNameForGraphQL)(requiredFieldName);
                            const field = input.getField(sanitizedFieldName);
                            if (!(0, graphql_1.isNonNullType)(field.type.getType()) && !field.extensions?.nullable) {
                                input.makeFieldNonNull(requiredFieldName);
                            }
                        }
                    }
                    if (isList) {
...
function isUnspecificType(typeComposer) {
